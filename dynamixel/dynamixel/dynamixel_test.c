...
main(int argc, char *argv[])
{
    ...
    getopt_add_string(gopt, 'd', "device", "/dev/ttyACM0", "USB Dynamixel device path");
    ...
    getopt_add_int(gopt, '\0', "LED", "0", "Set the servo's led color (0-7)");
    getopt_add_int(gopt, '\0', "KP", "32", "Set the servo's KP");
    getopt_add_int(gopt, '\0', "KD", "0", "Set the servo's KD");
    getopt_add_int(gopt, '\0', "KI", "0", "Set the servo's KI");
    getopt_add_int(gopt, '\0', "punch", "32", "Set the servo's punch");
    ...
    if (getopt_get_bool(gopt, "home-all")) {
        ...
        if (getopt_was_specified(gopt, "KP")){
            dynamixel_msg_t *msg = dynamixel_msg_create(3);
            msg->buf[0] = 29;
            msg->buf[1] = 0;
            msg->buf[2] = getopt_get_int(gopt, "KP") & 0xFF;
            dynamixel_msg_dump(msg);
            servo->write_to_RAM(servo,msg,0);
            dynamixel_msg_destroy(msg);
        }

        if (getopt_was_specified(gopt, "KD")){
            dynamixel_msg_t *msg = dynamixel_msg_create(3);
            msg->buf[0] = 27;
            msg->buf[1] = 28;
            msg->buf[2] = getopt_get_int(gopt, "KD") & 0xFF;
            servo->write_to_RAM(servo,msg,0);
            dynamixel_msg_destroy(msg);
        }

        if (getopt_was_specified(gopt, "KI")){
            dynamixel_msg_t *msg = dynamixel_msg_create(3);
            msg->buf[0] = 28;
            msg->buf[1] = 28;
            msg->buf[2] = getopt_get_int(gopt, "KI") & 0xFF;
            servo->write_to_RAM(servo,msg,0);
            dynamixel_msg_destroy(msg);
        }

        if (getopt_was_specified(gopt, "LED")){
            dynamixel_msg_t *msg = dynamixel_msg_create(3);
            msg->buf[0] = 25;
            msg->buf[1] = 0;
            msg->buf[2] = getopt_get_int(gopt, "LED") & 0xFF;
            servo->write_to_RAM(servo,msg,0);
            dynamixel_msg_destroy(msg);
        }

        if (getopt_was_specified(gopt, "punch")){
            dynamixel_msg_t *msg = dynamixel_msg_create(4);
            msg->buf[0] = 51;
            msg->buf[1] = 51;
            msg->buf[2] = getopt_get_int(gopt, "punch") & 0xFF;
            msg->buf[3] = (getopt_get_int(gopt, "punch") > 8) & 0xFF;
            servo->write_to_RAM(servo,msg,0);
            dynamixel_msg_destroy(msg);
        }
